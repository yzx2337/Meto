...
string dataset = "/home/yzx/dgy/ycsb/uniform-200/load200m.txt";
	...
	auto test = [&HashTable, &pop, &keys, &values, &method, &_v, &pool, &finished](int from, int to, int a_bucket)
	{
		...
		for (int i = from; i < to; i++)
		{
			...
			if (method[i] == 'i')
			{
				...
				++finished;
				if(finished % 10000000 == 0)
				{
					char ch;
					std::cout << "now: " << finished / 1000000 << " M" << std::endl;
					scanf("%c", &ch);
				}
			}
			else if (method[i] == 'r')
