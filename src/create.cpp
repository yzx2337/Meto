...
int main (int argc, char* argv[])
{
    ...
    if(access(path, 0) != 0){
	pop = pmemobj_create(path, "CCEH", POOL_SIZE, 0666);   //创建一个POOL_SIZE大小的文件
	if(!pop){
	    perror("pmemoj_create");
	    exit(1);
	}
	HashTable = POBJ_ROOT(pop, CCEH);     //POBJ_ROOT创建一个根对象，类型为CCEH
    ...
    ifstream ifs;
    string dataset = "/home/dell/yzx/src-code/CCEH/CCEH-PMDK/bin/input.txt";
    ifs.open(dataset);
    if (!ifs){
	cerr << "No file." << endl;
	exit(1);
