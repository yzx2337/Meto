...
int main(int argc, char *argv[])
{
	...
	auto run = [&HashTable, &pop, &keys, &values, &method, &_v, &pool, &searchFailed](int from, int to,int a_bucket, int idx)
	{
		int fail_cnt = 0;
		cpu_set_t my_set;
		CPU_ZERO(&my_set);
    ...
    auto neg = [&HashTable, &pop, &keys, &values, &method, &_v, &pool, &searchFailed](int from, int to,int a_bucket, int idx)
	{
		int fail_cnt = 0;
		cpu_set_t my_set;
		CPU_ZERO(&my_set);
	...
	cout << "total failed: " << totalFailed << endl;
	...
	clear_cache();
	clock_gettime(CLOCK_MONOTONIC, &start);
    ReInsertThreads.clear();

	for (int i = 0; i < numThreads; i++)
	{
		if (i != numThreads - 1)
			// ReInsertThreads.emplace_back(thread(test, chunk_size * i, chunk_size * (i + 1), i + MERGEBUCKET_SIZE));
			ReInsertThreads.emplace_back(thread(neg, chunk_size * i, chunk_size * (i + 1),i + MERGEBUCKET_SIZE, i));
		else
			// ReInsertThreads.emplace_back(thread(test, chunk_size * i, opernum, i + MERGEBUCKET_SIZE));
			ReInsertThreads.emplace_back(thread(neg, chunk_size * i, opernum,i + MERGEBUCKET_SIZE, i));
	}
	for (auto &t : ReInsertThreads)
		t.join();
	clock_gettime(CLOCK_MONOTONIC, &end);
