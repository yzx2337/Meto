...
void dram() {
    ...
    std::cout << "DRAM: " << rss*1.0/1024/1024 << " GB\n";
}
...
int main(int argc, char *argv[])
{
	...
	ifstream ifs;
string dataset = "/mnt/ycsb/zipfian-200/load200m.txt";
string readDs = "/mnt/ycsb/zipfian-200/read200m.txt";
	uint64_t *rkeys = new uint64_t[numData];
	ifs.open(readDs);
	if (!ifs)
	{
		cerr << "No file." << endl;
	...
	auto test = [&HashTable, &pop, &keys, &values, &method, &_v, &pool](int from, int to, int a_bucket)
	{
		...
		// string s = std::string((const char *)(method));
		// cout << s << endl;
	...
	{ // 改
		...
		// scanf("%d", &t);
		auto pos = [&HashTable, &pop, &rkeys, &searchFailed, &values](int from, int to, int tid)
		{
			int fail_cnt = 0;
			for (int i = from; i < to; i++)
			{
				...
				auto start_t = std::chrono::system_clock::now();
	#endif
				auto ret = D_RW(HashTable)->Get(rkeys[i]); // pos
	#ifdef LATENCY_TEST
				auto end_tt = std::chrono::system_clock::now();
				auto t_lat = std::chrono::duration_cast<std::chrono::nanoseconds>(end_tt - start_t);
				latency[i] = (double)(t_lat.count());
	#endif
				if (ret != reinterpret_cast<Value_t>(rkeys[i])) // pos,neg
		...
		auto neg = [&HashTable, &pop, &rkeys, &searchFailed, &values](int from, int to, int tid)
		{
			int fail_cnt = 0;
			for (int i = from; i < to; i++)
			{
				...
				auto start_t = std::chrono::system_clock::now();
	#endif
				auto ret = D_RW(HashTable)->Get(rkeys[i] - 1); // neg
	#ifdef LATENCY_TEST
				auto end_tt = std::chrono::system_clock::now();
				auto t_lat = std::chrono::duration_cast<std::chrono::nanoseconds>(end_tt - start_t);
				latency[i] = (double)(t_lat.count());
