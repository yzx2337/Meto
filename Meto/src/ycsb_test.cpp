...
int main (int argc, char* argv[])
{
    ...
    ifstream ifs;
    string dataset = "/home/dell/yzx/ycsb/uniform/load.txt";
    ifs.open(dataset);
    if (!ifs){
	cerr << "No file." << endl;
	exit(1);
	...
	// elapsed = (end.tv_sec - start.tv_sec)*1000000000 + (end.tv_nsec - start.tv_nsec);
	// cout << elapsed/1000 << "\tusec\t" << (uint64_t)(1000000*(opernum/(elapsed/1000.0))) << "\tOps/sec\tRun" << endl;
	// cout << "find:\t" << D_RW(HashTable)->find << "\tget_find:\t" <<  D_RW(HashTable)->get_find  << endl;

	vector<thread> UpdateThreads;
	dataset = "/home/dell/yzx/ycsb/uniform/insert.txt";
    ifs.open(dataset);
	keys = new uint64_t[opernum];
	method = new char[opernum];
	values = new Value_t [opernum];
	...
	for(int i=0; i<numThreads; i++){
		if(i != numThreads-1)
		    UpdateThreads.emplace_back(thread(test, chunk_size*i, chunk_size*(i+1),i+MERGEBUCKET_SIZE));
		else
		    UpdateThreads.emplace_back(thread(test, chunk_size*i, opernum,i+MERGEBUCKET_SIZE));
	}
